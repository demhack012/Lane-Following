...
def average_slope_intercept(image, lines):
    ...
    for line in lines:
        ...
        if y1 > 500:
            if np.abs(x1 - 670) < 350:
                ...
                if slope < 0:
                    left_far_fit.append((slope, intercept))
                    # print(left_fit)
                else:
                    right_far_fit.append((slope, intercept))
                    # print(right_fit)
        else:
            if np.abs(x1 - 670) < 300:
                if slope < 0:
                    left_near_fit.append((slope, intercept))
                else:
                    right_near_fit.append((slope, intercept))
    ...
    # print(left_line)
    # print(right_line)
    return np.array([left_far_line, right_far_line, left_near_line, right_near_line])
...
def region_of_interest(image):
    ...
    polygons = np.array(
        [[(0, height), (width, height), (width, 500), (530, 250), (0, 500)]]
    )
